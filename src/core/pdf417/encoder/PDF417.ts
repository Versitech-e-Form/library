...
export default class PDF417 {
  ...
  private static START_PATTERN = 0x1fea8;
  private static STOP_PATTERN = 0x3fa29;
  ...
  private static PREFERRED_RATIO = 3.0;
  private static DEFAULT_MODULE_WIDTH = 0.357; // 1px in mm
  private static HEIGHT = 2.0; // mm
  ...
  private static calculateNumberOfRows(m: number, k: number, c: number): number {
  ...
  private static getNumberOfPadCodewords(m: number, k: number, c: number, r: number): number {
    let n = c * r - k;
    return n > m + 1 ? n - m - 1 : 0;
  }

  private static encodeChar(pattern: number, len: number, logic: BarcodeRow): void {
    let map = 1 << len - 1;
  ...
  private encodeLowLevel(fullCodewords: string,
    ...
    for (let y = 0; y < r; y++) {
      ...
      for (let x = 0; x < c; x++) {
        pattern = PDF417.CODEWORD_TABLE[cluster][fullCodewords.charCodeAt(idx)];
        PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
        idx++;
      }
  ...
  public generateBarcodeLogic(msg: string, errorCorrectionLevel: number) {

    let errorCorrectionCodeWords = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);
    let highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding);
    let sourceCodeWords = highLevel.length;

    let dimension: Array<number> = this.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);

    let cols = dimension[0];
    let rows = dimension[1];

    let pad = PDF417.getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
    ...
    let n: char = sourceCodeWords + pad + 1;
    let sb: StringBuilder = new StringBuilder();
    sb.append(String.fromCharCode(n));
    sb.append(highLevel);
    for (let i = 0; i < pad; i++) {
      let num: char = 900;
      sb.append(String.fromCharCode(num)); // PAD characters
    }
...
// 4. step: low-level encoding
  ...
  private determineDimensions(sourceCodeWords: number, errorCorrectionCodeWords: number): Array<number> {
    let ratio = 0.0;
    let dimension: Array<number> = null;

    for (let cols = this.minCols; cols <= this.maxCols; cols++) {

      let rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
      ...
      let newRatio = ((17 * cols + 69) * PDF417.DEFAULT_MODULE_WIDTH
        ) /
        (rows * PDF417.HEIGHT);
...
// Handle case when min values were larger than necessary
    if (dimension == null) {
      ...
      let rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
      if (rows < this.minRows) {
        dimension = [this.minCols, this.minRows];
      }
    }
  ...
  public setDimensions(maxCols: number, minCols: number, maxRows: number, minRows: number): void {
