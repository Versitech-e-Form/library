...
export default class PDF417ErrorCorrection {
  ...
  private static EC_COEFFICIENTS: number[][] = [
    [27, 917],
    [522, 568, 723, 809],
  ...
  static getErrorCorrectionCodewordCount(errorCorrectionLevel: number): number {
    if (errorCorrectionLevel < 0 || errorCorrectionLevel > 8) {
      throw new Error('IllegalArgumentException("Error correction level must be between 0 and 8!");');
      // throw new IllegalArgumentException("Error correction level must be between 0 and 8!");
    }
    return 1 << (errorCorrectionLevel + 1);
  }

  static getRecommendedMinimumErrorCorrectionLevel(n: number): number {
    if (n <= 0) {
      throw new Error('IllegalArgumentException("n must be > 0");');
  ...
  static generateErrorCorrection(dataCodewords: string, errorCorrectionLevel: number): string {
    let k = this.getErrorCorrectionCodewordCount(errorCorrectionLevel);
    let e = new Array<char>(k);
    e.fill(0);
    let sld = dataCodewords.length;
    for (let i = 0; i < sld; i++) {
      let t1 = (dataCodewords.charCodeAt(i) + e[e.length - 1]) % 929;
      let t2: number;
      let t3: number;
      for (let j = k - 1; j >= 1; j--) {
        t2 = (t1 * PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][j]) % 929;
        t3 = 929 - t2;
        e[j] = <char>((e[j - 1] + t3) % 929);
      }
      t2 = (t1 * PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][0]) % 929;
      t3 = 929 - t2;
      e[0] = <char>(t3 % 929);
    }
